...
| 文件存储 | Supabase Storage | 开发者文件上传、自定义页面资源 |
...
3. 点击 **Save**
4. 进入 **Deployments** 标签，找到最新的部署，点击 **Retry deployment**（或推送任意 commit 到 GitHub 触发重新部署）
...
> **不需要配置 R2**。文件上传功能走 Supabase Storage，不需要 Cloudflare R2。
...
- **Supabase (PostgreSQL)**：聊天消息、表情反应、未读计数、实时推送
