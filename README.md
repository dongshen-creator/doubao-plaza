...
| 文件存储 | Cloudflare R2 + img.api.aa1.cn 图床 | 聊天图片/文件上传（img.api.aa1.cn 优先 + R2 兜底）+ 开发者文件托管（Supabase Storage） |
...
6. 创建 Storage 存储桶：
   ...
   > 只需要创建 `pages` 一个桶。聊天图片上传优先使用 img.api.aa1.cn 图床，R2 作为兜底，不需要额外存储桶。
...
> R2 绑定后，聊天上传的文件存储在 `chat-assets/{userId}/{timestamp}-{filename}` 路径下，通过 `/cdn-assets/` 路径访问。图片上传优先使用 img.api.aa1.cn 图床（免费、无需 API Key），R2 作为兜底方案。
...
- 图片/文件上传（img.api.aa1.cn 图床优先 + Cloudflare R2 兜底）
...
- **Cloudflare R2**：聊天图片/文件上传兜底存储（img.api.aa1.cn 图床失败时回退使用，通过 `/cdn-assets/` 路径访问）
...
A: 图片上传优先使用 img.api.aa1.cn 图床（免费无需 API Key），失败时回退到 Cloudflare R2。若 R2 上传成功但 URL 返回 404，检查 Cloudflare Pages 设置中是否已绑定 R2 存储桶（变量名 `PAGES_BUCKET`），并确认 `_routes.json` 中 `include` 数组包含 `/cdn-assets/*`。
